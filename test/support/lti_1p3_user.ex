...
defimpl Lti_1p3.Roles.Lti_1p3_User, for: Lti_1p3.Test.Lti_1p3_UserMock do
  def get_platform_roles(user) do
    user.platform_roles
    |> Lti_1p3.Roles.PlatformRoles.get_roles_by_uris()
  end

  def get_context_roles(user, context) do
    if context == "some-test-context" do
      user.context_roles
      |> Lti_1p3.Roles.ContextRoles.get_roles_by_uris()
    else
      []
    end
  end
