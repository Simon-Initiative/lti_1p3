defmodule Lti_1p3.Roles.PlatformRolesTest do
  ...
  describe "platform roles" do
    ...
    test "get_roles_by_uris returns all valid roles from a list of uris" do
      roles =
        PlatformRoles.get_roles_by_uris([
          "http://purl.imsglobal.org/vocab/lis/v2/institution/person#Instructor",
          "http://purl.imsglobal.org/vocab/lis/v2/institution/person#Learner"
        ])
    ...
    test "contains_role? returns true if a list of roles contains a given role" do
      ...
      assert PlatformRoles.contains_role?(roles, PlatformRoles.get_role(:institution_instructor)) ==
               true

      assert PlatformRoles.contains_role?(roles, PlatformRoles.get_role(:institution_learner)) ==
               true

      assert PlatformRoles.contains_role?(roles, PlatformRoles.get_role(:institution_mentor)) ==
               false
    end
    ...
    test "get_highest_role returns the highest level role from a list of roles" do
      ...
      assert PlatformRoles.get_highest_role(roles) ==
               PlatformRoles.get_role(:institution_instructor)
    ...
    test "has_role? returns true if a user has a given role" do
      ...
      assert PlatformRoles.has_role?(user, PlatformRoles.get_role(:institution_instructor)) ==
               false
    ...
    test "has_roles? with :any returns true if a user has any of the given roles" do
      ...
      assert PlatformRoles.has_roles?(
               user,
               [PlatformRoles.get_role(:institution_instructor)],
               :any
             ) == false
      ...
      assert PlatformRoles.has_roles?(user, [PlatformRoles.get_role(:institution_learner)], :any) ==
               true
